...
<div class="row">
	<div class="col-md-12">
		@if ($ticket->status != 'Withdrawn' && $ticket->next_approval_id && Auth::user()->person == $ticket->next_approval_id)
		<h2 class="mt-5 title-desc" style="text-align:center">You need to approve/reject this ticket.  </h2>
		@else
		<h2 class="mt-5 title-desc" style="text-align:center">You have done your action (approve/reject) for this ticket. Approve/reject action is unavailable.</h2>
		@endif
		<h2 style="text-align:center">Ticket Number : {{$ticket->ref}}</h2>
		<h2 style="text-align:center">Requester : {{$name}}</h2>
		<h2 style="text-align:center">Subject : {{$ticket->title}} </h2>
			<div style="    text-align: center;">
					@if ($ticket->status != 'Withdrawn' && $ticket->next_approval_id && Auth::user()->person == $ticket->next_approval_id)
						&nbsp;
						<button style="width: 200px;background-color: #3fb618;border-color: #3fb618;" class="btn btn-success btn-lg mt-7 reasonModalButton" data-id="{{ $ticket->id }}" data-title="{{ $ticket->title }}" data-status-ticket="Approve">Approve</button>
						&nbsp;
                ...
                if ($('.status-value').val() == 'approve') {
					...
					$.ajax({
						...
						success: function(data){
							...
							$('.title-desc').html('You have done your action (approve/reject) for this ticket. ');
							KTApp.unblockPage();
