			...
			<div class="card  pt-9 pr-1 pb-3 card-left" id="kt_card_10">
				<div class="card-header pt-1">
					<div class="card-title mb-0" style="float: left;display: block;">
						<h3 class="card-label">Goods Receive Category</h3>
					</div>
					...
					<ul class="nav flex-column nav-pills">
						...
						@foreach ($departments as $department)
							...
							//echo ucfirst($type).($type == "service"?" Request":"");
								$service_category2 = DB::table('service_category')->where('type', 'goods_receive')->where('department', $department->id)->get();
								$count_active_service_in_department = DB::table('lnkservicetoservice_category')
									...
									->where('service.request_type',ucwords(str_replace('_',' ', $type)).($type == "service"?" Request":""))
												...
												<div class="card-body">
													...
													<ul class="nav flex-column nav-pills">
														@foreach ($service_category2 as $category)
															...
															@if($count_cat)
																<li class="nav-item mb-2">
																	<a class="category-item-left nav-link link-{{$i}}" data-toggle="tab" href="#target-{{ $category->id }}" id="category-item-left-{{$category->id}}">
																		{{ $category->name }}
																	</a>
																</li>
															@endif
		...
		<div class="col-right col-md-8">
			<div class="card pt-6 pl-8">
				<div class="sticky-main">
					...
					<div class="tab-content d-none" id="myTabContent5">
						<?php $i=0;?>
							...
							<div class="row pr-8 pb-8">
								...
								<div class="col-md-4 col-xxl-4 col-lg-4">
									...
									<div class="card card-custom card-shadowless">
										@php
											$uri = URL('/').'/goods_receive/create?category='.$category->id.'&request='.$service->id.'&target=target-'.str_replace(' ','_',$category->name).'&inventory_category='.$type;

											if(!empty(request()->query('ticket'))) $uri .= '&ticket='.request()->query('ticket');
										@endphp
										<a href="{{$uri}}">
							...
							<ul class="d-none dashboard-tabs nav nav-pills nav-danger row row-paddingless m-0 p-0 flex-column flex-sm-row" role="tablist">
								...
								<li class="nav-item d-flex col flex-grow-1 flex-shrink-0 mr-3 mb-3 mb-lg-0">
									@php
										...
										if(!empty(request()->query('ticket'))) $uri .= '&ticket='.request()->query('ticket');
									@endphp
									<a class="nav-link border-nabati py-10 d-flex flex-grow-1 rounded flex-column align-items-center" href="{{ $uri }}">
...
<script>
    $(document).ready(function () {
        $('.category-item-left').click(function (e) {
            $('#myTabContent5').removeClass('d-none');
        });
    });
</script>
@endsection
...
<script>
	const el = document.querySelectorAll("a[href='target-26']")
	console.log(el, "EL")
</script>
