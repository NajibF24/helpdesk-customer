        ...
        <div>
            <div class="row mt-4">
                ...
                <div class="mb-2  col-md-4">
                    <span style="font-weight: 600" class="text-dark-75 text-hover-primary font-size-lg mb-1">Goods Issue</span>
                    <br> {{@$detail->goods_issue->code}} <br> <br>
...
<div  class="modal-reply" >
    <div class="modal-dialog modal-lg modal-dialog-centered" style="min-width: 1000px" role="document">
        ...
        <div class="modal-header">
            ...
            <form id="comment-form" method="post" action="{{URL('/inventory/comment')}}?type=goods_receive"  enctype="multipart/form-data">
                @csrf
...
$( document ).ready(function() {
    ...
    $('.approved').on('click', function() {
        ...
        var is_alr_custom = "{{ $is_alr_first_support_custom }}";
        var selectedMaterials = $('input[name="issue_detail_ids[]"]:checked').map(function() {
            return this.value;
        }).get();

        var materialConditions = $('input[name="conditions[]"]').map(function() {
            return this.value;
        }).get();

        var storeLocationIds = $('select[name="store_location_ids[]"]').map(function() {
            return this.value;
        }).get();

        // Check if at least one checkbox is checked
        if (selectedMaterials.length === 0 && is_alr_custom !== '') {
            Swal.fire({
                title: "Error",
                text: "Please select at least one material to approve.",
                icon: "error"
            });
            return;
        }
        ...
        Swal.fire({title: "Confirmation",text: "Are you sure want to approve ?",icon: "question",showCancelButton: true,confirmButtonText: "Yes!"
            ...
            if(result.value) {
                showLoading();
                $.ajax({
                    url: '{{ url("approve-request/ticketActionInventory") }}?id={{$detail->id}}&type=receive&action_detail=approved',
                    type: 'post',
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                    },
                    data: {
                        issue_detail_ids: selectedMaterials.join(','),
    ...
    $('.rejected').on('click', function() {
        ...
        Swal.fire({title: "Confirmation",text: "Are you sure want to Reject ?",icon: "question",showCancelButton: true,confirmButtonText: "Yes!"
            ...
            if(result.value) {
                $.ajax({
                    url: '{{ route("approve_request.reject_goods_receive", $detail->id) }}',
                    type: 'post',
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                    },
